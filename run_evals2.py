...
async def run_evals():
    ...
    for case in data['evals']:
        ...
        events = []
        async for evt in workflow.run(input_data, context=ctx):
            events.append(evt)
        ...
        if isinstance(final_record, dict):
            ...
            if 'assessment' in final_record:
                print(final_record['assessment'])
            if 'assessed_severity' in final_record:
                 print(f"Severity: {final_record.get('assessed_severity')}, Confidence: {final_record.get('confidence')}, Supports Claim: {final_record.get('evidence_supports_claim')}")
            print(f"Reasoning: {final_record.get('reasoning')}")
